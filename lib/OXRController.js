...
class OXRController {
  ...
  exchange(options = {}) {
    if (!options.hasOwnProperty('currency') || !options.hasOwnProperty('amount')) {
      throw new TypeError(
        'Argument to exchange function must be an object with the structure: ' +
        '{ currencty: <string>, amount: <float> }'
      );
    }
    ...
    return parseFloat((options.amount / this.rate(options.currency)).toFixed(4));
  }
}

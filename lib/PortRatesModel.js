...
class PortRatesModel {
  ...
  addItem(data) {
    if (!data.hasOwnProperty('item') || !data.hasOwnProperty('country')) {
      throw new TypeError(
        'argument must be an object with keys for item and country'
      );
    }

    if (!data.item.hasOwnProperty('outlier')) {
      // or calculate outlier status here for value.
      throw new TypeError('Outlier must be calculated for value.');
    }

    const rates          = this.rates;
    const ratesByCountry = this.ratesByCountry;

    rates.push(data.item);

    if (typeof ratesByCountry[data.country] === 'undefined') {
      ratesByCountry[data.country] = {rates: []};
    }

    ratesByCountry[data.country].rates.push(data.item);
